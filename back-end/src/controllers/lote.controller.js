    ...
    for (let x = 0; x < keys.length; x++) {
      ...
      for (let e = 0; e < inputs.length; e++) {
        ...
        if (keys[x] == "int") {
          ...
          if (data[keys[x]][inputs[e]]["value"] == "" || data[keys[x]][inputs[e]]["value"] == undefined) {
            errros[inputs[e]] = referencia + " no puede estar vacío";
          } else if (!/^\d+$/.test(data[keys[x]][inputs[e]]["value"])) {
            errros[inputs[e]] = referencia + " debe ser un número entero";
          } else {
            result[inputs[e]] = parseInt(data[keys[x]][inputs[e]]["value"]);
          }
        }
        if (keys[x] == "float") {
...
export const guardarlote = async (req, res) => {
  ...
  try {
    let data = {
      ...
      "int": {
        "n_plantas": {
          "value": req.body.n_plantas,
          "referencia": "El numero de plantas"
        }
      },
      "float": {
        "latitud": {
  ...
  try {
    ...
    let data = {
      ...
      "int": {
        ...
        "fincas_id": {
          "value": req.body.fincas_id,
          "referencia": "El ID de la finca"
        }
      }
    };
    let validateInputs = validate(data);
